...
mini-batch学习
从全部数据中选出一部分，作为全部数据的“近似”。神经网络的学习也是从训练数据中选出一批数据（称为mini-batch, 小
批量），然后对每个mini-batch 进行学习。比如，从60000 个训练数据中随机
...
w = w11 w12 w13 #权重参数
	w21 w22 w23
#aL是损失函数
aL/aw = aL/aw11 aL/aw12 aL/aw13 #这就是梯度
...
for i in range(iters_num):
	# 获取mini-batch
	batch_mask = np.random.choice(train_size, batch_size)
	x_batch = x_train[batch_mask]#从10000个训练数据中抽取100个
	t_batch = t_train[batch_mask]#从10000个监督数据中抽取了100个
	...
	for key in ('W1', 'b1', 'W2', 'b2'):
		network.params[keys] = learning_rate * grad[key]
